...
struct ImageClient {
    ...
    // takes in a url string and uses a completion handler to capture processed image from the url source e.g "https://.....jpg"
    
    // we cannot simply return an image from this function, why?
    // because URLSession performs an asynchronous network call, which means the call relies on the network and is done on the background, not performed instantaneously
